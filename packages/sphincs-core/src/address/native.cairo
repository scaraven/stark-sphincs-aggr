// Modified version of the address module, uses felt252 in the underlying representation
...
pub struct Address {
    w0: felt252, // layer, hypertree address
    w1: felt252, // hypertree address
    w2: felt252, // hypertree address, address type
    w3: felt252, // keypair high/low bytes
    w4: felt252, // tree height | wots chain address
    w5: felt252, // tree index | wots hash address (we use lower bytes)
    // Cached values
    w0_a: felt252,
    w0_bcd: felt252,
    w2_a: felt252,
...
pub impl AddressImpl of AddressTrait {
    ...
    fn set_hypertree_addr(ref self: Address, tree_address: u64) {
        ...
        self.w0_bcd = abc.into();
        self.w0 = self.w0_a + self.w0_bcd;
        self.w1 = defg.into();
        self.w2_a = h.into() * 0x1000000;
    ...
    fn to_word_array(self: @Address) -> WordArray {
        WordArrayTrait::new(array![(*self.w0).try_into().unwrap(), (*self.w1).try_into().unwrap(), (*self.w2).try_into().unwrap(), (*self.w3).try_into().unwrap(), (*self.w4).try_into().unwrap()], (*self.w5).try_into().unwrap(), 2)
    }

    fn into_components(self: Address) -> (u32, u32, u32, u32, u32, u32, u32, u32) {
        (self.w0.try_into().unwrap(), self.w1.try_into().unwrap(), self.w2.try_into().unwrap(), 
        self.w3.try_into().unwrap(), self.w4.try_into().unwrap(), self.w5.try_into().unwrap(), 0_u32, 0_u32)
    }
