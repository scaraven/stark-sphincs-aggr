...
fn split_xdigest_128s(digest: felt252) -> XMessageDigest {
    ...
    let (g_high, g_low) = DivRem::div_rem(g_mhash, 0x100); // g_high=top 2 bytes, g_low=bottom byte
    ...
    let arr: Array<u32> = array![
        ...
        e_high + d_low * 0x10000, // d[bot 2B] || e[top 2B]
        f_high + e_low * 0x10000, // e[bot 2B] || f[top 2B]
        g_high + f_low * 0x10000, // f[bot 2B] || g[16:32] (2B)   
    ];

    // Append g_low as the final byte
    let mhash = WordSpanTrait::new(arr.span(), g_low, 1);
...
mod tests {
    ...
    fn test_split_xdigest_128s() {
        ...
        assert_eq!(xdigest.tree_address, 0x101d68);
        assert_eq!(words_to_hex(xdigest.mhash), "337bbad9e4a1621e38c5e3827d8ae84c41");
    }
}
